...
description: >-
  Create Excalidraw diagram JSON files that make visual arguments. Use when
  the user wants to visualize workflows, architectures, or concepts. Generates
  .excalidraw JSON and renders to PNG for validation.
---
...
Colors encode information, not decoration. Every color choice should come from `color-palette.md` in the references directory -- the semantic shape colors, text hierarchy colors, and evidence artifact colors are all defined there.
...
See `element-templates.md` in the references directory for copy-paste JSON templates for each element type (text, line, dot, rectangle, arrow). Pull colors from `color-palette.md` based on each element's semantic purpose.
...
First, locate the render script:

```
Glob: **/excalidraw-diagram/references/render_excalidraw.py
```

Then run from that directory:

```bash
cd <render script directory> && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
...
If the render script hasn't been set up yet, locate the references directory and run:

```bash
cd <references directory> && uv sync && uv run playwright install chromium
```
...
Based on [excalidraw-diagram-skill](https://github.com/coleam00/excalidraw-diagram-skill) by Cole Medin. Adapted to the-no-shop plugin format with Glob-based reference discovery for portable installation.
